...
const SitterApprove = () => {
    ...
    axios.post('http://localhost:3001/api/accept-babysitter', { name, email, password })
        .then(() => {
            alert(`${name} accepted!`);
   ...
   const handleReject = (name, email) => {
    if (window.confirm(`Are you sure you want to reject ${name}?`)) {
        axios.post('http://localhost:3001/api/reject-babysitter', { name, email })
            .then(() => {
                setBabysitters(prev => prev.filter(b => b.name !== name));
    ...
    return (
        <div style={styles.container}>
            <h1 style={styles.header}>Babysitter Applications</h1>
